...
integer     :: rate, itime_start, itime_end, itime_setup
...
!and md_run.inp
  !write(*,*)' read in the MD input '
  write(*,*)' my rank is ', para_var%my_rank, ' out of ', para_var%size_rank, ' before_read_in_md '
  CALL read_in_md(md, para_var)
! ------------------------------------------- !
  ! ----------------------------------- !
...
if (para_var%my_rank == 0) then
  ...
  open(111,file='runtime_info.dat', position="append")
    write(111,*)' set up run time is ', itime_setup - itime_start, real(itime_setup-itime_start)/real(rate)
  close(111)
endif
