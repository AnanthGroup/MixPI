...
REAL(KIND=8),PARAMETER, PUBLIC        :: rhydberg=10973731.568157 !rhydberg constant in 1/m
REAL(KIND=8),PARAMETER, PUBLIC        :: h_planck=6.62607015E-34 !planck constant J*s
REAL(KIND=8),PARAMETER, PUBLIC        :: joule=2.0d0*rhydberg*h_planck*c_light !hartree to joule
REAL(KIND=8),PARAMETER, PUBLIC        :: kb=1.3806504E-23/joule !3.1668114e-6       !Eh/K  !exact until conversion
...
TYPE md_info
  ...
  REAL(kind=8),ALLOCATABLE,DIMENSION(:)              ::xx_rcut
  CHARACTER(LEN=2),ALLOCATABLE,DIMENSION(:)          ::xx_types
  LOGICAL,ALLOCATABLE,DIMENSION(:)                   ::xx_exclude
  INTEGER                                            ::xx_atom_num
  INTEGER                                            ::run_average
END TYPE md_info
